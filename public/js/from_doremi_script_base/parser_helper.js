...
(function() {
  ...
  root.ParserHelper = {
    ...
    parse_line: function(uppers, sargam, lowers, lyrics) {
      var attribute_lines, ctr, item, lower, lyric, my_items, my_line, my_lowers, my_uppers, upper, x, _i, _j, _k, _l, _len, _len1, _len2, _len3;
      this.line_warnings = [];
      if (lyrics.length === 0) {
        lyrics = '';
      }
      ...
      for (_j = 0, _len1 = lowers.length; _j < _len1; _j++) {
        lower = lowers[_j];
        lower.group_line_no = ctr;
        ctr = ctr + 1;
      }
      for (_k = 0, _len2 = lyrics.length; _k < _len2; _k++) {
        lyric = lyrics[_k];
        lyric.group_line_no = ctr;
      }
      for (_l = 0, _len3 = my_items.length; _l < _len3; _l++) {
        my_line = my_items[_l];
    ...
    assign_syllables_from_lyrics_sections: function(composition) {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        if (line.my_type === "sargam_line") {
          _results.push(this.assign_lyrics2(line, syls));
        } else {
    ...
    parse_composition: function(attributes, lines) {
      var char, ctr, hash, line, lower, split_chars, to_string, valid, x, _i, _j, _len, _len1;
      ctr = 0;
      ...
      if (x && valid) {
        ...
        for (_j = 0, _len1 = split_chars.length; _j < _len1; _j++) {
          char = split_chars[_j];
    ...
    extract_lyrics: function() {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        sargam_line = _ref[_i];
        _ref1 = this.all_items(sargam_line, []);
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        _results.push((function() {
          ...
          for (_j = 0, _len1 = measures.length; _j < _len1; _j++) {
            ...
            if (measure.beat_count < 4) {
              this.log("setting is_partial true");
              this.log("inside if");
              _results1.push(measure.is_partial = true);
            } else {
    ...
    measure_pitch_durations: function(line) {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        if (item.my_type === "dash" && item.dash_to_tie) {
          ...
          _results.push(last_pitch.fraction_total = _.reduce(last_pitch.fraction_array, my_funct, null));
        } else {
          _results.push(void 0);
        }
      }
    ...
    measure_dashes_at_beginning_of_beats: function(line) {
      var all, beats, beats_with_dashes_at_start_of_beat, ctr, denominator, done, first_dash, item, last_pitch, m_beats, measure, measures, my_beat, _fn, _i, _j, _k, _l, _len, _len1, _len2, _len3, _ref, _ref1,
        _this = this;
      ...
      beats_with_dashes_at_start_of_beat = _.select(beats, function(beat) {
        var pitch_dashes;
        ...
        _this.log("pitch_dashes", pitch_dashes);
      ...
      for (_j = 0, _len1 = beats_with_dashes_at_start_of_beat.length; _j < _len1; _j++) {
    ...
    measure_note_durations: function(beat) {
      var ctr, denominator, len, microbeats, _i, _results,
        _this = this;
      ...
      ctr = 0;
      _results = [];
      for (ctr = _i = 0; 0 <= len ? _i < len : _i > len; ctr = 0 <= len ? ++_i : --_i) {
        _results.push((function(ctr) {
          var done, item, numerator, x, _fn, _j, _ref, _ref1;
          item = beat.items[ctr];
          _this.log("in do loop, item is " + item.source);
          _this.log("in do loop, ctr is " + ctr);
          if (item.my_type !== "pitch") {
            return;
          }
          _this.log("setting denominator for " + item.source);
          numerator = 1;
          done = false;
          if (ctr < len) {
            _fn = function(x) {
              ...
              if (x < len) {
                ...
                if (!done) {
                  ...
                  _this.log("in inner loop, my_item is" + my_item.source);
                  return _this.log("in inner loop, x is" + x);
                }
              }
            };
            for (x = _j = _ref = ctr + 1, _ref1 = len - 1; _ref <= _ref1 ? _j <= _ref1 : _j >= _ref1; x = _ref <= _ref1 ? ++_j : --_j) {
              _fn(x);
            }
          }
          _this.log("setting numerator,denominator for " + item.source);
    ...
    find_ornaments: function(attribute_lines) {
      var item, line, orn_item, orns, _i, _j, _k, _len, _len1, _len2, _ref, _ref1;
      orns = [];
    ...
    map_ornaments: function(ornaments) {
      var column, map, orn, ornament_item, _i, _j, _len, _len1, _ref;
      map = {};
      ...
      for (_i = 0, _len = attribute_lines.length; _i < _len; _i++) {
        ...
        _results.push((function() {
          ...
          for (column in attribute_nodes) {
            ...
            _results1.push((function(column, attribute) {
              var orn_obj, sarg_obj;
              _this.log("processing column,attribute", column, attribute);
              ...
              if (_this.check_semantics(sargam, sarg_obj, attribute, sargam_nodes) === !false) {
    ...
    my_inspect: function(obj) {
      ...
      return console.log("my_inspect:", obj);
    }
  };
