$(document).ready ->
  ...
  window.CompositionViewModel = (my_doremi_source) ->
    ...
    self.compute_doremi_source = () ->
      #return "hi"
      # console.log "entering compute_doremi_source"
  ...
  window.timed_count = () =>
    ...
    if composition.last_doremi_source isnt composition.doremi_source() # the source changed
      ...
      if !parsed?
        ...
        which_line.line_has_warnings(false) if which_line?
        which_line.line_warnings([])
        which_line.line_parse_failed(true) if which_line?
      else
        which_line.line_parse_failed(false) if which_line?
